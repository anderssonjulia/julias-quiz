...
def start_round(input_manager, user, question_list):
    ...
    question_board = QuestionBoard.filter_by_selected_difficulty(selected_difficulty, question_list)
    question_board.randomize()

    score = 0
    while len(question_board) > 0:
        question = question_board.pop_question()
        user_question_answer = input_manager.ask_question(question)
...
def main():
    ...
    user = User(username=input_manager.get_user_name(), score=0)
